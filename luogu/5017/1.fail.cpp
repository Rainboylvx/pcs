...
int main(){
        ...
        //if ( m == 2) 
    {
        ...
        for(i=2;i<=idx;i++){
            ...
            //debug("\n-=== 第 %d 个人\n",i);
            if(t[i] - t[i-1] >=m){
                for(j=t[i-1];j<=t[i-1]+m-1;j++)
                    base = min(base,f[j]);
                for(j=t[i];j>=max(t[i]-2*m,(ll)0);j--)
                    f[j] = base;
                for(j=t[i];j<=t[i]+m-1;j++){
                    f[j] = 0x7f7f7f7f7f7f7f7f;
                    for(k =max(0,j-2*m);k<=j-m;k++)
        ...
        for(i=t[idx]+m-1;i >= t[1] && i >= t[idx] ;i--){
            ans = min(ans,f[i]);
        }
        printf("%lld\n",ans);
    }
