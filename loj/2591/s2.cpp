...
struct _node{ int x,y,val,select; 
...
void dfs(int dep,int f){
    ...
    int i,j,_min = 999,idx;
    For(i,1,t_cnt){ //找到最小 0的数量的点
        if( node[i].select ) continue;
        if( _min > node[i].val ){
            _min = node[i].val;
            idx  = i ;
        }
    ...
    For(i,1,9){
        ...
        //最优性剪枝
        if( f+i*fen[x][y] + 90*(t_cnt - dep) <= ans) break;
        ...
        For(j,1,t_cnt){
            if( node[j].select) continue;
            if( node[j].x == node[idx].x) 
                node[j].val--;
            else if ( node[j].y == node[idx].y)
                node[j].val--;
            else if( g_num(x, y) == g_num(node[j].x, node[j].y))
                node[j].val--;
        }
