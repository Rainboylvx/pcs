...
* time: 2020年 05月 18日 星期一 17:40:57 CST
* problem: loj-10103
*----------------*/
#include <bits/stdc++.h>
            ...
            if( !dfn[v]){
                ...
                if( low[v] >=dfn[u] ){
                    ...
                    t=-1; do {
                        ...
                        if( color_n_t[u] != nbcc_cnt && cut_n[u]) cnt[u]++,color_n_t[u] = nbcc_cnt,ans =max(cnt[u],ans);
                        if( color_n_t[v] != nbcc_cnt && cut_n[v]) cnt[v]++,color_n_t[v] = nbcc_cnt,ans = max(cnt[v],ans);
                    }while( t != i );
                }
#endif
            }
