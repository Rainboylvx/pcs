...
var con = fs.readFileSync("./省选斗兽场-NOI神殿.md",{encoding:"utf8"}).split('\n')
...
for( let p of con){
  if( /P(\d{4})/.test(p)){
    ...
    let title =  getInfo(num).currentTitle.replace('[','').replace(']','')
    console.error("get title",num,title)

    if( fs.existsSync("../../luogu/"+num) ) {
      let ret = e(`curl -s http://127.0.0.1:4046/utils/exists/luogu-P${num}`)
      if( /true/.test(ret)) {
